# This code is a modified version of code provided by FNJPNews. https://github.com/FNJPNews/UEFNDownloader
#!/usr/bin/env python3
"""
Single-file CLI for:
...
def render_progress(done: int, total: int, *, label: str = "Downloading game data") -> None:
    if total <= 0:
        return
    width = 30
...
def aes_keychain_string(guid: str, aes_key_hex: str) -> str:
    """Same format as UEFN-AES-Dumper-CS: KeyChain: <GUID(no-dash,upper)>:<base64(AES bytes)>"""
...
def prompt_map_code(raw: str | None) -> str:
    ...
    while True:
        value = input("Enter map code: ").strip()
        try:
...
def device_code_login_and_create_device_auth(*, data_dir: Path, timeout: float) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    # Note: this endpoint does not accept application/json;
    # it must be x-www-form-urlencoded or it returns 415 Unsupported Media Type.
...
def resolve_content_token(args: argparse.Namespace) -> tuple[str | None, str | None]:
    ...
    # First try to silently log in / refresh using the device_auth.json saved in STEP3.
    # (The manual flow that opens a browser and pastes a 32-char code is not used here.)
    try:
...
def cmd_device_login(args: argparse.Namespace) -> int:
    ...
    print("Logged in")
    print(f"Username: {device_auth.get('displayName', '<unknown>')}")
...
def probe_diagnostic(results: list[dict[str, Any]]) -> str:
    ...
    if any("operation_forbidden" in error for error in errors):
        return (
            "The Content Service project API returned operation_forbidden."
            "This API cannot be read without project owner/team permissions even for public islands,"
            "so the artifact ID cannot be resolved from the mnemonic projectId alone."
        )
    if any("Jwt is missing" in error or "invalid_token" in error for error in errors):
        return (
            "fn-gateway does not accept this OAuth token as a JWT."
            "A different token derived from the Fortnite client may be required."
        )
    return "No artifact ID candidate was found in the Content Service response."
...
def map_code_download_blocker(results: list[dict[str, Any]], mnemonic: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "Downloading this public island requires the Fortnite GameCustom InstallBundle, not the UEFN project API.",
        "In practice it fetches BuildPatchServices chunks from plugin.manifest and reconstructs plugin.pak / plugin.ucas / plugin.utoc / plugin.sig.",
        "",
        f"Values known from mnemonic: projectId={project_id}, link/version={version}",
        "Missing values: root module id / module version / bundle id",
        "",
        "The Fortnite client resolves these missing values via ContentBeacon after matchmaking.",
        "The manifest URL cannot be constructed from the mnemonic API and an unauthorized Content Service project API alone.",
        "",
        "The current main download path uses the Content Service v4 cooked-content-package resolver.",
    ]
    if not public_modules:
        lines.insert(
            6,
            "public_modules in the mnemonic response is empty. If module info were present we could proceed, but it is not returned for this island.",
        )
    return "\n".join(lines)
...
def cmd_game_version(args: argparse.Namespace) -> int:
    """
    Prints the current UEFN/Fortnite game version in 'major.minor' format (e.g. 41.10)
    as a single line to stdout. Called from the C# side (DecrypterSettingsCommand)
    to determine whether a signature re-scan is needed. Lightweight and requires no auth.
    """
    try:
...
# ---------------------------------------------------------------------------
#
#  Detection strategy (robust across versions):
#
#  The target function does not directly reference "Ias.EncryptionKey=".
#  Instead it is located via two steps:
#
#  Step 1: Find RIP-relative LEA/MOV xrefs to the "Ias.EncryptionKey=" string
#          -> Walk backwards to find the start of the "parent function" containing that instruction
#
#  Step 2: Follow the first CALL rel32 (E8) from the start of the parent function
#          -> That destination is the target function (the one that receives the AES key)
#
#  This relationship is an invariant call structure derived from the UE engine key-parsing flow;
#  it is preserved across versions as long as the compiler does not optimize or inline it.

def _sig_read_pe_sections(data: bytes):
    """Returns a list of PE file sections: [(virt_addr, raw_off, raw_size, name), ...]"""
    if data[:2] != b"MZ":
...
def _sig_find_string_rvas(data: bytes, sections, needle: str) -> set[int]:
    """Returns all RVAs where needle (ASCII / UTF-16LE) exists."""
    rvas: set[int] = set()
...
def _sig_find_xref_offsets(data: bytes, sections, string_rvas: set[int]) -> list[int]:
    """
    Scans the .text section and returns a list of file offsets for
    RIP-relative LEA/MOV instructions that reference any of the string_rvas.
    """
    refs: list[int] = []
...
def _sig_find_func_start(data: bytes, sections, from_foff: int, max_back: int = 0x2000) -> int | None:
    """
    Walks backwards from from_foff and returns the nearest function boundary.
    Boundary detection:
      - Immediately after 2+ consecutive INT3 (0xCC) bytes (alignment padding)
      - When a REX/PUSH-type byte follows RET (0xC3) with no padding
    """
    for vaddr, raw_off, raw_size, _ in sections:
...
def _sig_follow_first_call(data: bytes, sections, func_start: int, max_scan: int = 0x200) -> int | None:
    """
    Returns the file offset of the destination of the first CALL rel32 (E8)
    found within max_scan bytes of func_start.
    """
...
def _sig_log(msg: str) -> None:
    """Writes progress logs to stderr in real time (stdout is reserved for results)."""
...
def cmd_find_signature(args: "argparse.Namespace") -> int:
    ...
    # ── Step 1: xref to "Ias.EncryptionKey=" -> start of parent function ──
    ...
    # ── Step 2: first CALL rel32 in parent function -> target function ────
